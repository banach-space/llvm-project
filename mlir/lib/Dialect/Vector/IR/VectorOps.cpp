  ...
  for (auto [idx, pos] : llvm::enumerate(position)) {
    if (auto attr = dyn_cast<Attribute>(pos)) {
      ...
      if (!isValidPositiveIndexOrPoison(
              constIdx, kPoisonIndex, getSourceVectorType().getBaseDimSize(idx))) {
...
OpFoldResult vector::ShuffleOp::fold(FoldAdaptor adaptor) {
  ...
  auto mask = getMask();
  if (isStepIndexArray(mask, 0, v1Type.getBaseDimSize(0)))
    return getV1();
  // Fold shuffle V1, V2, [4, 5] : <4xi32>, <2xi32> -> V2.
  if (isStepIndexArray(mask, v1Type.getDimSize(0), v2Type.getBaseDimSize(0)))
    return getV2();
  ...
  for (auto [idx, pos] : llvm::enumerate(position)) {
    if (auto attr = pos.dyn_cast<Attribute>()) {
      ...
      if (!isValidPositiveIndexOrPoison(constIdx, kPoisonIndex,
                                        destVectorType.getBaseDimSize(idx))) {
        return emitOpError("expected position attribute #")
               << (idx + 1)
               << " to be a non-negative integer smaller than the "
                  "corresponding "
...
LogicalResult ScatterOp::verify() {
  ...
  if (llvm::size(getIndices()) != memType.getRank())
    return emitOpError("requires ") << memType.getRank() << " indices";
  if (valueVType.getBaseShape() != indVType.getBaseShape())
    return emitOpError("expected valueToStore dim to match indices dim");
  if (valueVType.getBaseShape() != maskVType.getBaseShape())
